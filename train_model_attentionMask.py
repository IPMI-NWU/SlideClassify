...
from models.cls_model_multi_drop import BAL_P, BAL_A
...
parser.add_argument('--datasetsName', type=str, default='nsclc', help='directory to save features')
...
def train_epoch(epoch, model, optimizer, trainloader, criterion, first_round):
    model.train()
    target_wsi_all = []
    attns = {}
    loss_nn_all = 0.
    for i_batch, sampled_batch in enumerate(trainloader):
        ...
        target = target.cuda()
        if first_round:
            logit, Y_prob, Y_hat, attn = model(input,status='val')
        else:
            logit, Y_prob, Y_hat, attn = model(input, status='val')
...
if __name__ == '__main__':
    ...
    if not args.is_test:
        ...
        for epoch in range(args.epochs):
            train_attns = train_epoch(epoch, model, optimizer, trainloader, criterion, first_round=True)
        ...
        ###mask attention
        for epoch in range(args.epochs):
            train_attns = train_epoch(epoch, model, optimizer, trainloader, criterion, first_round=False)
            valid_loss, val_auc, val_attns, _, _ = prediction(model, valloader, criterion)
            early_stopping(epoch, valid_loss, best_acc, model)
            if early_stopping.early_stop:
                print()
