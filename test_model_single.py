...
parser.add_argument('--datasetsName', type=str, default='jiaozhiliu', help='directory to save features')
...
parser.add_argument('--k', type=str, default=1, help='fold to ues')
...
def BestTarget(predicted, target_wsi,threshold):
    ...
    if flag == 0:
        ...
        best_threshold = thresholds[bestACC_index]
    ...
    return accs[bestACC_index], best_threshold

def prediction(model, loader, criterion,threshold=-1, testing="Val"):
    model.eval()
    ...
    with torch.no_grad():
        for i_batch, sampled_batch in enumerate(loader):
            ...
            logit, Y_prob, Y_hat, attn = model(input)
            result_csv.append([img_id[0],target.item()])
    ...
    if args.n_classes == 2:
        acc, threshold = BestTarget(output, target_wsi, threshold)
        predicted = torch.tensor(output)
        predicted_labels = (predicted[:, 1] > threshold).int()
        print(f'using threshold {threshold}')
        for i in range(len(result_csv)):
            result_csv[i].append(predicted_labels[i].item())
    else:
