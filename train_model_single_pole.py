...
parser.add_argument('--datasetsName', type=str, default='pole', help='directory to save features')
parser.add_argument('--n_classes', type=int, default=2, help='num of classes for slide')  #singel 固定为2
...
parser.add_argument('--seed', type=int, default=3047, help='random seed for reproducible experiment')
...
def BestTarget(predicted, target_wsi):
    ...
    print()
    return accs[bestACC_index]

def prediction(model, loader, criterion, testing="Val"):
    model.eval()
    ...
    if args.n_classes == 2:
        acc = BestTarget(output, target_wsi)
    else:
    ...
    torch.cuda.empty_cache()
    return total_loss, roc_auc, attns, result_csv
...
if __name__ == '__main__':
    ...
    if args.model == "AB-MIL":
        model = BAL_P(n_classes=2, input_dim=args.encoding_size, subtyping=False).to(device)
    ...
    if not args.is_test:
        ...
        for epoch in range(args.epochs):
            ...
            valid_loss, val_auc, val_attns, _ = prediction(model, valloader, criterion)
            print(f"{valid_loss} add to lossCount!")
    ...
    _, test_acc, test_attns, test_result = prediction(model, testloader, criterion, testing="Test")
    _, _, train_attns, train_result = prediction(model, trainloader, criterion, testing="Train")
    _, _, val_attns, val_result = prediction(model, valloader, criterion, testing="Val")
